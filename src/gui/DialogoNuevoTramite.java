...
public class DialogoNuevoTramite extends JDialog implements ActionListener {
	private static final long serialVersionUID = 4582284500903177972L;
	//hola
	...
	public DialogoNuevoTramite(VentanaPrincipal ventanaPrincipal) {	
		super(ventanaPrincipal, "Nuevo Trámite", true);
		setLayout(null);
		...
		tramite = new Tramite();
		panelDatosGenerales = new PanelDatosGenerales(tramite);
		panelDatosGenerales.setBounds(10, 10, 990, 590);
		panelDatosEspecificos = new PanelDatosEspecificos(tramite);
		panelDatosEspecificos.setBounds(10, 10, 990, 590);
		panelPasos = new PanelPasos(tramite);
	...
	private void inicializar() {
		...
		panelDatosEspecificos.ocultarComponentes();
		panelPasos.ocultarComponentes();
	}
	...
	private void aceptar() {
		try {
			JOptionPane.showMessageDialog(this, "El sistema se está preparando para crear el trámite."
					+ "\nEsta operación puede durar algunos segundos "
					+ "\ndependiendo de la cantidad y tamaño de las plantillas seleccionadas", 
					"Nuevo trámite", JOptionPane.INFORMATION_MESSAGE);
			ventanaPrincipal.getLista().setTramite(tramite);
			// crear base de datos
			ventanaPrincipal.getLista().crearTramite();
			JOptionPane.showMessageDialog(this, "Trámite creado con éxito"
					+ "\n\nTrámite: " + tramite.getNombreTramite() 
					+ "\nDepartamento o área: " + tramite.getDepartamento() 
					+ "\n" + tramite.getDescripcion(), "Nuevo trámite", 
