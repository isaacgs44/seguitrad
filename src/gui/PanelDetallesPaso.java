...
public class PanelDetallesPaso extends JPanel {
	...
	//REPLACE
	/**
	 * Guarda el trámite que incluye los pasos específicos obligatorios y con fecha límite.
	 */
	 ...
	 *            trámite específico para obtener los pasos específicos.
	 * 
	 * @see #inicializar()
	 */
	public PanelDetallesPaso(ArrayList<Paso> pasos,
		...
		for (Paso p : pasos) {
			if (p.isObligatorio() && p.isConFechaLimite()) {
				...
				if (tramiteEspecifico == null || tramiteEspecifico.buscarPasoEspecifico(p.getNombrePaso(), p.getRepeticion()) == -1) {//REPLACE
					...
					//Desactiva la modificación si el paso ya fue realizado
					if (paso.isRealizado()) {
